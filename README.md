# MIA Final Project: 結合 3D 跨模態 Swin Transformer 及任務感知多任務學習腦腫瘤分割和存活預測輔助診斷系統

這是一個基於深度學習的醫學影像分析 (MIA, Medical Image Analysis) 專案，主要針對 BraTS2020 腦瘤 MRI 影像數據集進行多任務學習和可解釋性人工智慧 (XAI) 的分析。
...
資料集包含四種 MRI 模態影像：T1、T1Gd、T2、FLAIR，以及相關的臨床存活天數及病患資訊。

## 專案架構與核心功能

專案分為兩個主要的執行腳本，分別對應不同的訓練策略和模型架構：

### 1. `main.py` - 多任務 Cross-Modal 模型及 Baseline 比較
這是專案的主要流程，實作一個**多任務 3D 網路**，並提供 Baseline 模型的比較功能。
- **主要模型 (Proposed):** Cross-Modal Swin-Transformer 3D
  - 透過 Cross-Modal Attention 融合 T1/T1Gd 和 T2/FLAIR 模態的特徵。
...
- **可解釋性 AI 功能:**
  ...
  - 影像重疊對比：黃色輪廓代表官方真實標註，淺藍色輪廓代表模型預測，幫助直觀檢視模型預測結果的準確度。
- **評估指標:** 主要評估 Tumor Region 的 IoU 以及 Dice 分數。
